...
class StrategyMetrics:
    ...
    def calculate_metrics(self, trades: List[float], 
        ...
        # Obliczanie Sharpe Ratio z obsługą std == 0
        if len(returns) > 1:
            std_dev = returns.std()
            if std_dev is not None and np.isfinite(std_dev) and std_dev > 1e-9: # Sprawdź czy std jest skończone i większe od zera
                self.sharpe_ratio = returns.mean() / std_dev
            else:
                # Jeśli std_dev jest 0, NaN, Inf lub bardzo bliskie zera, Sharpe jest nieokreślony
                self.sharpe_ratio = np.nan
        else:
            self.sharpe_ratio = np.nan # Sharpe nieokreślony dla <= 1 transakcji

        # Obliczanie Sortino Ratio z obsługą std == 0 dla strat
        downside_returns = returns[returns < 0]
        if len(downside_returns) > 1:
            downside_std_dev = downside_returns.std()
            if downside_std_dev is not None and np.isfinite(downside_std_dev) and downside_std_dev > 1e-9: # Sprawdź czy std jest skończone i większe od zera
                self.sortino_ratio = returns.mean() / downside_std_dev
            else:
                # Jeśli std_dev strat jest 0, NaN, Inf lub bardzo bliskie zera, Sortino jest nieokreślony
                self.sortino_ratio = np.nan
        else:
            # Jeśli nie ma strat lub jest tylko jedna, Sortino jest nieokreślony
            self.sortino_ratio = np.nan
