...
def filter_unique_strategies(results: List[Dict],
    ...
    try:
        ...
        # Wybór najlepszej strategii z każdego klastra
        unique_strategies_orig_indices = []
        processed_clusters = set()
        for i, label in enumerate(cluster_labels): # i to indeks w danych po walidacji
            if label in processed_clusters: continue
            indices_in_cluster = [idx for idx, cl_label in enumerate(cluster_labels) if cl_label == label]
            if not indices_in_cluster: processed_clusters.add(label); continue

            # Znajdź najlepszą wg score WŚRÓD tych w klastrze
            ...
            if best_new_idx_in_cluster != -1:
                 ...
                 unique_strategies_orig_indices.append(best_original_idx)
