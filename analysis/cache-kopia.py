...
class AnalysisCache:
    ...
    def __init__(self):
        ...
        self._strategy_groups: Dict[str, List[Dict]] = {} # { symbol: [ group1, group2, ... ] }
    ...
    @property
    def strategy_groups(self) -> Dict:
        """Zwraca słownik z grupami strategii."""
        return self._strategy_groups
    ...
    def get_groups(self, symbol: str, results: List[Dict]) -> List[Dict]:
        """
        Zwraca (i oblicza jeśli trzeba) grupy strategii dla danego symbolu
        używając metody z reports.py (hashowanie metryk).
        """
        cache_key = f"strategy_groups_{symbol}"
        if symbol not in self._strategy_groups:
            logger.debug(f"[{symbol}] Cache MISS: Obliczam grupy strategii (metoda z reports.py)...")
            start_time = time.time()
            # Import lokalny
            from analysis.reports import group_similar_strategies
            # group_similar_strategies zwraca listę grup
            self._strategy_groups[symbol] = group_similar_strategies(results) # Zakładamy, że results są dla TEGO symbolu
            self._last_update[cache_key] = time.time()
            logger.debug(f"[{symbol}] Cache SET: Zapisano {len(self._strategy_groups[symbol])} grup strategii (czas: {time.time() - start_time:.2f}s)")
        else:
            logger.debug(f"[{symbol}] Cache HIT: Zwracam grupy strategii.")
        return self._strategy_groups.get(symbol, [])
    ...
    def clear(self):
        ...
        start_size = (len(self._param_distributions) + len(self._strategy_groups) +
        ...
        gc.collect() # Dodatkowo wywołaj GC
        logger.info(f"Cache wyczyszczony (usunięto wpisy dla {start_size} kluczy).")