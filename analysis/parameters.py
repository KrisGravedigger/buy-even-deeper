...
def analyze_parameter_distributions(ranked_results: List[Dict], n_top: int = 100) -> Dict:
    ...
    for symbol, symbol_strategies in results_by_symbol.items():
        ...
        # Jawne zwolnienie pamięci po przetworzeniu symbolu
        if 'df' in locals():
            del df
            # logger.debug(f"[{symbol}] Zwolniono DataFrame dla dystrybucji.") # Opcjonalny log
...
def filter_unique_strategies(results: List[Dict],
    ...
    param_data = np.array(param_data_rows, dtype=float) # Upewnij się, że typ to float
    metric_data = np.array(metric_data_rows, dtype=float) # Upewnij się, że typ to float

    # --- Obsługa NaN/Inf PRZED skalowaniem/PCA ---
    # Użyj np.nan_to_num do zastąpienia NaN zerami, a Inf dużymi liczbami
    # To zapobiega błędom w StandardScaler/PCA/KMeans
    param_data = np.nan_to_num(param_data, nan=0.0, posinf=np.finfo(np.float64).max/10, neginf=np.finfo(np.float64).min/10)
    metric_data = np.nan_to_num(metric_data, nan=0.0, posinf=np.finfo(np.float64).max/10, neginf=np.finfo(np.float64).min/10)
    logger.debug(f"Zastosowano np.nan_to_num do danych parametrów i metryk.")


    # --- Normalizacja, PCA, KMeans ---
    try:
        # Sprawdź, czy nadal są kolumny po konwersji i czy jest więcej niż 1 wiersz
        if param_data.shape[1] > 0 and param_data.shape[0] > 1:
             param_norm = StandardScaler().fit_transform(param_data)
        else: param_norm = param_data # Użyj danych po nan_to_num jeśli nie można skalować
        if metric_data.shape[1] > 0 and metric_data.shape[0] > 1:
             metric_norm = StandardScaler().fit_transform(metric_data)
        else: metric_norm = metric_data # Użyj danych po nan_to_num jeśli nie można skalować
        ...
        # W przypadku błędu, zwracamy niefiltrowane, ale poprawne wyniki
        return [results[i] for i in valid_results_indices]
    finally:
        # --- Blok finally dla pewności zwolnienia pamięci ---
        # Usuwamy duże tablice NumPy, jeśli zostały utworzone, niezależnie od sukcesu czy błędu w try
        variables_to_del = [
            'param_data', 'metric_data', 'param_norm', 'metric_norm',
            'param_reduced', 'metric_reduced', 'combined_data', 'cluster_labels',
            'kmeans' # Również model KMeans może zajmować pamięć
        ]
        cleaned_count = 0
        for var_name in variables_to_del:
            if var_name in locals():
                try:
                    del locals()[var_name]
                    cleaned_count += 1
                except NameError: # Na wszelki wypadek
                    pass
        if cleaned_count > 0:
            logger.debug(f"filter_unique_strategies: Jawnie usunięto {cleaned_count} zmiennych w bloku finally.")
        # logger.debug("filter_unique_strategies: Wykonano blok finally.") # Opcjonalny log
