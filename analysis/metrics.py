...
def calculate_strategy_score(metrics: Dict) -> float:
    ...
    # Sprawdź typy przed operacjami arytmetycznymi, dopuszczając typy NumPy i None
    basic_metrics_values = [expectancy, profit_per_trade, win_rate, sharpe, sortino]
    valid_types = True
    for v in basic_metrics_values:
        # Dopuść None oraz liczby (Pythonowe i NumPy)
        if v is not None and not isinstance(v, (int, float, np.number)):
            valid_types = False
            break

    if not valid_types:
        # Zmieniamy logikę - loguj ostrzeżenie, ale próbuj liczyć dalej,
        # zastępując niepoprawne wartości None (lub 0.0).
        logger.warning(f"Niektóre podstawowe metryki mają nieoczekiwane typy lub wartości (zostaną potraktowane jako 0 lub None w obliczeniach score): {metrics}")
        # Zastąp niepoprawne wartości None, aby dalsze obliczenia były bezpieczniejsze
        def sanitize(val):
            if val is None or isinstance(val, (int, float, np.number)):
                # Obsługa NaN i Inf w numpy/float
                if isinstance(val, (float, np.number)) and not np.isfinite(val):
                    return None # Traktuj NaN/Inf jako None dla obliczeń score
                return val
            return None # Zwróć None dla innych niepoprawnych typów

        expectancy = sanitize(expectancy)
        profit_per_trade = sanitize(profit_per_trade)
        win_rate = sanitize(win_rate)
        sharpe = sanitize(sharpe)
        sortino = sanitize(sortino)
        # Kontynuuj obliczenia, ale z None tam, gdzie były złe typy.
        # Dalsza część kodu musi obsługiwać None.

    # Dodatkowa obsługa None w dalszej części obliczeń score:
    # Zastąp None zerami PRZED obliczeniami (prostsze)
    expectancy = expectancy if expectancy is not None else 0.0
    profit_per_trade = profit_per_trade if profit_per_trade is not None else 0.0
    ...
    # Podobnie dla innych metryk używanych w score
    max_dd = metrics.get('max_drawdown', 100.0)
    # Użyj wartości domyślnej (np. 100.0 dla max_dd) dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(max_dd, (int, float, np.number)) or not np.isfinite(max_dd):
        max_dd = 100.0

    sl_blocks = metrics.get('stop_loss_blocks', 0) # Zakładamy, że to int
    sl_eff = metrics.get('stop_loss_block_effectiveness') # Może być None
    # Użyj 0.0 dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(sl_eff, (int, float, np.number)) or not np.isfinite(sl_eff):
        sl_eff = 0.0

    btc_blocks = metrics.get('btc_blocks', 0) # Zakładamy, że to int
    btc_eff = metrics.get('btc_block_effectiveness') # Może być None
    # Użyj 0.0 dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(btc_eff, (int, float, np.number)) or not np.isfinite(btc_eff):
        btc_eff = 0.0

    btc_corr_val = metrics.get('btc_correlation') # Może być None
    # Użyj 0.0 dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(btc_corr_val, (int, float, np.number)) or not np.isfinite(btc_corr_val):
        btc_corr = 0.0
    else:
        btc_corr = abs(btc_corr_val)

    btc_rate_val = metrics.get('btc_block_rate') # Może być None
    # Użyj 0.0 dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(btc_rate_val, (int, float, np.number)) or not np.isfinite(btc_rate_val):
        btc_rate = 0.0
    else:
        btc_rate = btc_rate_val

    trade_count_val = metrics.get('trade_count') # Może być None
    # Użyj 0 dla None/niepoprawnych typów, ale dopuść np.integer
    if not isinstance(trade_count_val, (int, np.integer)):
        trade_count = 0
    else:
        trade_count = trade_count_val

    max_loss_val = metrics.get('max_loss') # Może być None lub 0
    # Użyj 0.0 dla NaN/Inf/None/niepoprawnych typów
    if not isinstance(max_loss_val, (int, float, np.number)) or not np.isfinite(max_loss_val):
        max_loss = 0.0
    else:
        max_loss = max_loss_val

    profit_factor_val = metrics.get('profit_factor') # Może być None, inf, nan
    # Zachowajmy inf, ale zamieńmy nan/None/inne na 0 dla bezpieczeństwa porównań
    if isinstance(profit_factor_val, (int, float, np.number)):
        if np.isinf(profit_factor_val):
            profit_factor = profit_factor_val # Zachowaj inf
        elif np.isfinite(profit_factor_val):
            profit_factor = profit_factor_val
        else: # Jest NaN
            profit_factor = 0.0
    else: # Jest None lub inny typ
        profit_factor = 0.0

    risk_reward_val = metrics.get('risk_reward_ratio') # Może być None, inf, nan
    # Podobnie jak dla profit_factor
    if isinstance(risk_reward_val, (int, float, np.number)):
        if np.isinf(risk_reward_val):
            risk_reward = risk_reward_val # Zachowaj inf
        elif np.isfinite(risk_reward_val):
...
def analyze_single_strategy(result: Dict) -> Dict:
    ...
    try:
        ...
        # Konwertuj metryki na słownik, obsługując typy NumPy i NaN/Inf
        metrics_dict = {}
        for k, v in metrics_calculator.__dict__.items():
            if not k.startswith('_'):
                # Sprawdź typy NumPy i Pythonowe
                if isinstance(v, (int, float, str, bool, list, dict, tuple)) or v is None:
                    # Standardowe typy Pythona LUB None
                    # Dodatkowo sprawdź NaN/Inf w standardowych float
                    if isinstance(v, float):
                        if pd.isna(v): # Użyj pandas.isna dla kompatybilności
                            metrics_dict[k] = None # Zapisz NaN jako None
                        elif np.isinf(v):
                            metrics_dict[k] = str(v) # Zapisz inf jako string
                        else:
                            metrics_dict[k] = v # Poprawny float
                    else:
                        metrics_dict[k] = v # Inne poprawne typy lub None
                elif isinstance(v, np.number):
                    # Typy liczbowe NumPy (np. np.float64, np.int64)
                    # Sprawdź NaN/Inf przed konwersją do typu Pythonowego
                    if pd.isna(v): # Użyj pandas.isna dla kompatybilności
                        metrics_dict[k] = None # Zapisz np.nan jako None dla lepszej kompatybilności (np. z JSON)
                    elif np.isinf(v):
                        metrics_dict[k] = str(v) # Zapisz np.inf/-inf jako string ('inf', '-inf')
                    else:
                        # Konwertuj na standardowy typ Pythona
                        metrics_dict[k] = v.item()
                else:
                    # Inne, potencjalnie nieserializowalne typy
                    try:
                        # Spróbuj zapisać jako string jako ostateczność
                        metrics_dict[k] = str(v)
                    except Exception:
                        metrics_dict[k] = f"Unserializable type: {type(v).__name__}"
