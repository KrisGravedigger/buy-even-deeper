...
def analyze_strategies(results_with_metrics: List[Dict], processes: int, cache: AnalysisCache) -> Tuple[List[Dict], Dict, Dict]:
    ...
    for symbol in sorted(results_by_symbol.keys()):
        ...
        try:
            ...
            logger.debug(f"[{symbol}] Cache: Pobieram/Obliczam grupy strategii...")
            cache.get_groups(symbol, symbol_results) # Używa wszystkich dla symbolu
        except Exception as e:
        ...
        # Dodawanie informacji analitycznych (rankingi, metoda filtrowania)
